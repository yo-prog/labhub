...
void dialogstats::statistiques(QVector<double>* ticks,QVector<QString> *labels)
{
    QSqlQuery qry;
    int i=0;
    qry.exec("SELECT NOM FROM PROJET");
    while (qry.next())
    {
...
void dialogstats::MakeStat()
{
    ...
    // Couleurs
    A->setName("Repartition des projets selon leur période");
    A->setPen(QPen(QColor("#0000FF")));
    A->setBrush(QColor("#ADD8E6"));
    ...
    ui->plot->xAxis->setLabel("PROJETS");
    ...
    QSqlQuery q1("SELECT PERIODE FROM PROJET");
    while (q1.next())
    {
...
void dialogstats::on_pushButton_clicked()
{
    close();
}
